...
log.info "SNP nbhd	= ${snp_nbhd}"
log.info "Cval preproc	= ${cval_preproc}"
log.info "Cval proc 1	= ${cval_proc1}"
log.info "Cval proc 2	= ${cval_proc2}"
log.info "Min read count	= ${min_read_count}"
log.info "Ref	= ${params.ref}"
log.info "Dbsnp vcf ref	= ${params.dbsnp_vcf_ref}"
...
// Input folder with pairs of bam => Output: pairX.csv.gz
    ...
    '''
      !{params.snppileup_bin} --gzip --min-map-quality !{params.min_map_quality} --min-base-quality !{params.min_base_quality} --pseudo-snps !{params.pseudo_snps} --min-read-counts !{min_read_count} !{params.dbsnp_vcf_ref} !{tumor_normal_tag}.csv.gz !{tn[2]} !{tn[0]}
    '''
}
